...
if not exists(f'{path_to_download}/{file_name}.zip'):
    # unzip
    zipped_file = ZipFile(f'{path_to_download}/{file_name}')
...
for key, df in multisheet.items():
    ...
    for i, r in unique_vs.iterrows():
        ...
        sub_df_dict = sub_df.groupby(['Code System']).apply(lambda x: list(zip(x['Code'].tolist(),x['Description'].tolist()))).to_dict()
