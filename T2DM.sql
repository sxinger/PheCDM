...
phenotyping on a smaller cut of CDM tables relevant to the eligible patient cohort
*************************************************************************************************/
...
---------------------------------------------------------------------------------------------------------------
CREATE TABLE NextD_InclusionMeds_final AS
SELECT PATID
...
CREATE TABLE NextD_incl_restrict_meds AS
SELECT a.PATID
      ,a.RX_ORDER_DATE
      ,a.RAW_RX_MED_NAME
      ,a.RXNORM_CUI
...
--FROM nightherondata.visit_dimension -- add "@server" at the end if the "&&I2B2_ID" schema is on a different server 
--where start_date < Date '2010-01-01' -- KUMC specific 
--;

--when visit_dimension doesn't seem to be reliable 
CREATE TABLE NextD_EstablishedPatient AS
SELECT DISTINCT pat.PATID 
FROM NextD_first_visit pat
JOIN nightherondata.observation_fact obs -- add "@server" at the end if the "&&I2B2_ID" schema is on a different server 
ON pat.patid = obs.patient_num
where obs.start_date > pat.birth_date and -- dates of demographic facts usually mapped back to birth_dates
      obs.start_date < Date '2010-01-01' -- KUMC specific 
;
...
---------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------
drop table FinalStatsTable1_local purge;
...
SELECT v.PATID
      ...
      ,CASE WHEN estab.PATID IS NOT NULL or 
                 v.NextD_first_visit > dm.DMonsetDate
            THEN 1
            ELSE NULL
...
where dm.DMonsetDate >= Date '2010-01-01'
;

select count(distinct patid) from FinalStatsTable1_local
where DMonsetDate is not null; /*85,397*/

--QA check point
select * from FinalStatsTable1_local
where DMonsetDate < FirstVisit and ESTABLISHEDPATIENTFLAG is null
order by (FirstVisit-DMonsetDate) desc; /*1,642*/
...
---------------------------------------------------------------------------------------------------------------
drop table FinalStatsTable1 purge;
...
select count(*),count(distinct PATID) pat_all, /*444,282*/ /*496,634*/
       sum(EstablishedPatientFlag) pat_estab, /*147,190*/ /*158,180*/
       count(distinct PATID)-sum(EstablishedPatientFlag) pat_new /*297,092*/ /*338,454*/
from FinalStatsTable1;
